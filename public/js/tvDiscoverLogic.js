...
renderDiscoverGrid = (data) =>{
	for(let i=0;i<data.length;i++){
		...
		newEle.setAttribute("href","/tv/"+data[i].id)
		newEle.innerHTML = `
...
dateFrom.addEventListener("change",function(){
	saveItemStorage("&first_air_date.gte=",dateFrom);
})

dateTo.addEventListener("change",function(){
	saveItemStorage("&first_air_date.lte=",dateTo);
})
...
clearBtn.addEventListener("click",function(){
	...
	eleArr2 = [minScore,minVote,minRuntime,maxRuntime] 
	eleArr3 = [minScoreValue,minVoteValue,minRuntimeValue,maxRuntimeValue]
...
initFilterValue = () =>{
	let objArr = []
	let eleArr = [movieSortSelect,genreSelect,dateFrom,dateTo,minScore,minVote,minRuntime,maxRuntime]
	let keyArr = ["&sort_by=","&with_genres=","&first_air_date.gte=","&first_air_date.lte=","&vote_average.gte=","&vote_count.gte=","&with_runtime.gte=","&with_runtime.lte="]
	...
	for (let i=0;i<eleArr.length;i++){
		...
		if (keyArr[i] == "&sort_by=" || keyArr[i] == "&with_genres=" || keyArr[i] == "&first_air_date.gte=" || keyArr[i] == "&first_air_date.lte="){
			defaultValue = ""
		}else{
			defaultValue = 0;
		}
...
initShowFilterValue = () =>{
	let objArr = []
	let eleArr = [minScoreValue,minVoteValue,minRuntimeValue,maxRuntimeValue]
	let keyArr = ["&vote_average.gte=","&vote_count.gte=","&with_runtime.gte=","&with_runtime.lte="]
